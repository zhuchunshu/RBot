...
if (!function_exists("sendMsg")){
    function sendMsg($data, $action,$response_type="array")
    {
       go(function() use ($data, $action,$response_type){
           return sendData($data, $action,$response_type);
       });
    }
}

if (!function_exists("sendData")){
    function sendData($data, $action,$response_type="array"): \GuzzleHttp\Promise\PromiseInterface|array|string|\Psr\Http\Message\ResponseInterface|\Overtrue\Http\Support\Collection
    {
        $data['access_token']=get_options('access_token');
        $url = get_options('http',"http://127.0.0.1:5700");
        $url .= "/".$action . "?access_token=" . get_options('access_token');
        $data['access_token']=get_options('access_token');
...
if(!function_exists("RBotMsg")){
    function RBotMsg($msg): RBotMsg
    {
        return new RBotMsg($msg);
    }
}
 ...
 */
function base64_image_content($base64_image_content,$path): bool|string
{
    //匹配出图片的格式
    if (preg_match('/^(data:\s*image\/(\w+);base64,)/', $base64_image_content, $result)){
        $type = $result[2];
        $new_file = public_path("plugins/xuexi")."/";
        if(!is_dir(public_path("plugins"))){
            \Swoole\Coroutine\System::exec("mkdir ".public_path("plugins"));
        }
        if(!is_dir(public_path("plugins/xuexi"))){
            \Swoole\Coroutine\System::exec("mkdir ".public_path("plugins/xuexi"));
        }
        $filename = time() . ".{$type}";
        $new_file .= $filename;
